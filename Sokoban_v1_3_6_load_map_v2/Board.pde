  ...
  void loadTiles() {
    ...
    { 
      ...
      for(int j = 0; j < pieces.length; j++)
      {
        ...
        if(value == 1)
        {
          newBoard[i][j] = new Tile(initxPos+i*offset, inityPos+j*offset, tileWidth, tileHeight, playColor);
        }
        else if(value == 2)
        {
          newBoard[i][j] = new Tile(initxPos+i*offset, inityPos+j*offset, tileWidth, tileHeight, 4);
        }
        else if(value == 3)
        {
          newBoard[i][j] = new Tile(initxPos+i*offset, inityPos+j*offset, tileWidth, tileHeight, 5);
        }
        else
        {
          newBoard[i][j] = new Tile(initxPos+i*offset, inityPos+j*offset, tileWidth, tileHeight, 1);
        }
